...
impl PyUserModel {
    ...
    /// Returns the data table whose output range contains the cell, as a
    /// dictionary, or None.
    pub fn get_data_table<'py>(
        &self,
        py: Python<'py>,
        sheet: u32,
        row: i32,
    ...
    pub fn set_data_table(
        ...
        self.model
            .set_data_table(sheet, range, row_input_cell, column_input_cell)
            .map_err(to_py_err)
    }
    ...
    /// Returns the workbook's iterative-calculation settings as a dictionary.
    pub fn get_iterative_calculation<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        to_python(py, self.model.get_iterative_calculation())
    }
